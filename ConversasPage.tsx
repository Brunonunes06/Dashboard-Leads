            ...
            <button key={lead.id} onClick={() => handleSelect(lead.id)}
              ...
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 bg-emerald-500/20 rounded-full flex items-center justify-center text-emerald-400 text-sm font-bold">{lead.name[0]}</div>
                  <span className="font-medium text-sm text-white truncate">{lead.name}</span>
                </div>
                <div className="flex items-center gap-1.5">
                  {lead.unread > 0 && <span className="bg-emerald-500 text-slate-950 text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">{lead.unread}</span>}
              ...
              <div className="flex items-center justify-between pl-10">
                ...
                <span className={`w-2 h-2 rounded-full ${statusColors[lead.status] ?? "bg-slate-500"}`} />
              </div>
            </button>
          ))}
        </div>
        ...
        <main className="flex-1 flex flex-col">
          <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 bg-emerald-500/20 rounded-full flex items-center justify-center text-emerald-400 font-bold">{selected.name[0]}</div>
              <div>
                <p className="font-semibold text-sm">{selected.name}</p>
                <p className="text-slate-400 text-xs">{selected.phone}</p>
              </div>
            </div>
            <span className="text-xs px-2.5 py-1 rounded-full bg-slate-700 text-slate-300 capitalize">{selected.status}</span>
          </div>

          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {selected.messages.map((msg) => {
              const isMine = (isAdmin && (msg.sender === "human" || msg.sender === "ai")) || (!isAdmin && msg.sender === "lead");
              return (
                <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[70%] rounded-2xl px-4 py-2.5 text-sm ${msg.sender === "ai" ? "bg-blue-500/20 border border-blue-500/30 text-blue-100" : isMine ? "bg-emerald-500 text-slate-950 rounded-br-sm" : "bg-slate-800 text-white rounded-bl-sm"}`}>
                    {msg.sender === "ai" && <p className="text-xs font-semibold mb-1 text-blue-400">🤖 IA</p>}
                    <p className="leading-relaxed">{msg.text}</p>
                    <p className={`text-xs mt-1 ${isMine && msg.sender !== "ai" ? "text-slate-800/70 text-right" : "text-slate-500"}`}>{msg.time}</p>
                  </div>
                </div>
              );
            })}
          ...
          <div className="px-6 py-4 border-t border-slate-800">
            ...
            <p className="text-slate-600 text-xs mt-2">Enter para enviar · Shift+Enter para nova linha</p>
          </div>
        </main>
      ) : (
        <main className="flex-1 flex items-center justify-center">
          <div className="text-center"><span className="text-5xl">💬</span><p className="text-slate-400 mt-3">Selecione uma conversa</p></div>
        </main>
      )}
    </div>
  );
