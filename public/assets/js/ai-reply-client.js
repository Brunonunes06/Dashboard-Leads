// Porta de src/lib/ai-reply-client.ts.
(function () {
  ...
  function buildAIReplyInput(lead, history) {
    const settings = getAISettings();
    const input = history.map((m) => ({
      role: m.sender_role === "client" ? "user" : "assistant",
      // A Responses API da OpenAI rejeita content:null com 400 — mensagens
      // sem texto (ex: mídia do WhatsApp sem legenda) viram string vazia.
      content: m.content || "",
    }));
    ...
    return [{ role: "system", content: systemContext }, ...input];
  }
  ...
  window.AIReplyClient = { HANDOFF_MARKER, getAISettings, buildAIReplyInput, splitHandoff };
})();
